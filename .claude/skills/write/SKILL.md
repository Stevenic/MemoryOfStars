...
- **Voice:** `writing-guide/voice.md` — write *in* it (flowing paragraphs with the drop as emphasis, emotion in the body, definition-by-negation, deadpan dialogue with beats attached, flatly-rendered uncanny, verbatim litany). Also `style-guide.md` (mechanics, the `⸻` scene break, POV/tense).
...
- **Voice is non-negotiable** — flowing paragraphs with the single-line drop saved for the beats that earn it; dialogue beats attached (never more than ~3 clipped exchanges without a body) with each character deflecting differently; **emotion carried by the body, never named** (no banned templates — voice.md §6); **speakers must see each other** — gaze written as a sequence with intent, a look allowed to be a full reply, hooks planted in the hands, rationed like salt (the choreography layer, voice.md §3); the uncanny stated flatly with dread built by repetition/counting; land the chapter on a short resonant line. Run `voice.md`'s 30-second self-check on your own draft.
